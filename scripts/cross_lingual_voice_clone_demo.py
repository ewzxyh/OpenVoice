...
BASE_SPEAKER_CONFIGS: Dict[str, Dict[str, object]] = {
    "english": {
        "checkpoint_candidates": [Path("checkpoints/base_speakers/EN")],
        "style_embeddings": {
    ...
    "chinese": {
        "checkpoint_candidates": [Path("checkpoints/base_speakers/ZH")],
        "style_embeddings": {
    ...
    "portuguese": {
        "checkpoint_candidates": [
            Path("checkpoints/base_speakers/PT_BR"),
            Path("checkpoints/base_speakers/PT"),
            Path("checkpoints/base_speakers/BR"),
        ],
        "style_embeddings": {
            "default": [
                "pt_br_default_se.pth",
                "pt_default_se.pth",
                "ptbr_default_se.pth",
                "br_default_se.pth",
            ],
...
def _normalise_candidates(value: object) -> list[str]:
...
def _resolve_checkpoint_dir(candidates: Iterable[Path]) -> Path:
    candidate_paths = [Path(candidate) for candidate in candidates]
    if not candidate_paths:
        raise FileNotFoundError("No checkpoint directories configured for language.")
...
def _resolve_style_embedding(
    checkpoint_dir: Path, style: str, candidates: Iterable[str]
) -> Path:
    for candidate in candidates:
        embedding_path = checkpoint_dir / candidate
        if embedding_path.is_file():
            return embedding_path
    candidate_list = ", ".join(str(checkpoint_dir / c) for c in candidates)
    raise FileNotFoundError(
        "Could not find a speaker embedding for style "
        f"'{style}'. Looked for: {candidate_list}"
    )
...
def _validate_checkpoint_files(config: Dict[str, object]) -> Path:
    ...
    checkpoint_dir = _resolve_checkpoint_dir(config["checkpoint_candidates"])
...
def _load_base_speaker(
    ...
    checkpoint_dir = _validate_checkpoint_files(config)

    style_embeddings: Dict[str, list[str]] = {
        style_name: _normalise_candidates(candidates)
        for style_name, candidates in config["style_embeddings"].items()
    }
    ...
    embedding_path = _resolve_style_embedding(
        checkpoint_dir, style, style_embeddings[style]
    )
    ...
    source_embedding = torch.load(embedding_path, map_location=device).to(device)

    friendly_language = config["friendly_name"]
    language_mark = config.get("language_mark")
    if language_mark:
        BaseSpeakerTTS.language_marks.setdefault(
            friendly_language.lower(), language_mark
        )
